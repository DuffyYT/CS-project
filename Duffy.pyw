...
from pip import main               #For custom entry widget
...
import ctypes                                #For dpi awarness
...
if os.path.isfile('data/mem.txt'):
    with open ('data/mem.txt', 'r') as f:
        memSplit = f.read()
...
class setting:
    ...
    def __init__(self, pad,one=None,two=None,three=None,four=None,five=None,six=None,seven=None,
            ...
            tlist = [(one),(two),(three),(four),(five),(six),
                               (seven),(eight),(nine),(zero),(dot),(dash)] #list of primary buttons with <black> background and <white> foreground
            clst = [(dele),(delete),(quit)]
            olst = [(ad),(sub),(mul),(div),(sq),(sqr)]
                    ...
                    try:
                        if texture != '':
                            ...
                            if theme == 'Dark':
                                ...
                                (self.eq).config(image=Deq)
                                (self.sq).config(image=Dsq)
                                (self.sqr).config(image=Dsqr)
                                settings["texture"] = texture
                                settings["ui"] = 'custom'
                        elif texture == '':
...
def setting1():
    setting(entryPad, num1, num2, num3, num4, num5, num6, num7, num8, num9, num0, decimal, Negative,settings1,Clear1,Clear2,Clear3,add,subtract,multiply,divide,Sqr,Sqrt,equals)
def setting2():
    setting(StentryPad, Stnum1, Stnum2, Stnum3, Stnum4, Stnum5, Stnum6, Stnum7, Stnum8, Stnum9, Stnum0, Stdecimal, StNegative,settings2,StC,StCE,StAC,Stadd,Stsubtract,Stmultiply,Stdivide,StSqr,StSqrt,Stequals)
def setting3():
    setting(SentryPad,Snum1,Snum2,Snum3,Snum4,Snum5,Snum6,Snum7,Snum8,Snum9,Snum0,Sdecimal,SNegative,settings3,SC,SCE,SAC,Sadd,Ssubtract,Smultiply,Sdivide,SSqr,SSqrt,Sequals)
def setting4():
    setting(WentryPad,Wnum1,Wnum2,Wnum3,Wnum4,Wnum5,Wnum6,Wnum7,Wnum8,Wnum9,Wnum0,Wdecimal,button=settings4,dele=WC,delete=WCE,quit=WAC)
def setting5():
    setting(TentryPad,Tnum1,Tnum2,Tnum3,Tnum4,Tnum5,Tnum6,Tnum7,Tnum8,Tnum9,Tnum0,Tdecimal,button=settings5,dele=TC,delete=TCE,quit=TAC)
...
def fHistory():
    root=tk.Tk()
    root.title("History")
    root.iconbitmap('resource\icon\calc1.ico')
    def Fpop():
        global counter
        memory.clear()
        if counter == 1:
...
def scientific():
 ...
 #Brrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr
    ...
    def tr(func,con = 'deg',mode = 'B',pre = None,n = None):
        try:
            print(con)
            x = float(field.get())
            field.delete(0,END)
            if con == 'deg':
                siner = func(math.radians(x))
            elif con == 'rad':
                siner = func(x)
            upperField.delete(0,END)
            upperField.insert(0,siner)
            upperAns.delete(0,END)
            upperAns.insert(0,f"{n}({x})")
            upperOperator.delete(0,END)
            upperOperator.insert(0,"📐")
            if mode == 'K':
                pre.config(relief=SUNKEN)
                pre.after(58, lambda: pre.config(relief=RAISED))
            memory.append(f'{n}({x})={siner}{con}')
        except:
            pass

    def SSin():
        tr(math.sin,trig,'K',sin)
    #  try:
    #     if trig == 'rad':
    #         x=float(field.get())
    #         siner = math.sin(x)
    ...
    keyboard.on_press_key("s", lambda _:tr(math.sin,trig,'K',sin,'sin'))
    keyboard.on_press_key("o", lambda _:tr(math.sin,trig,'K',cos,'cos'))
    keyboard.on_press_key("t", lambda _:tr(math.sin,trig,'K',tan,'tan'))
    #========================================#
    keyboard.on_press_key("i", lambda _:tr(Csc,trig,'K',csc,'csc'))
    keyboard.on_press_key("u", lambda _:tr(Sec,trig,'K',sec,'sec'))
    keyboard.on_press_key("j", lambda _:tr(Cot,trig,'K',cot,'cot'))
    #========================================#
...
def time():
 ...
 if tPad ==0:
    ...
    TAC = tk.Button(TentryPad, bg="#F24607", font="calibri 10", text="AC",command=ACT, activebackground="black", activeforeground="#F24607")
    TAC.place(relheight=0.17, relwidth=0.27, rely=0.73, relx=0.72)
...
if mainpad == 'entrypad' or mainpad == 'Stentrypad':
    print(mainpad)
    try:
        if maindir:
            if mainui == 'custom':
        ...
        if maindir:
            if mainui == 'custom':
                if mainthm == 'Dark':
                    ...
                    nine = ImageTk.PhotoImage(file=f"{maindir}/nine.png")
                    zero = ImageTk.PhotoImage(file=f"{maindir}/zero.png")
                    Snum1.config(image=one,bg="black")
                    Snum2.config(image=two,bg="black")
                    Snum3.config(image=three,bg="black")
                    Snum4.config(image=four,bg="black")
                    Snum5.config(image=five,bg="black")
                    Snum6.config(image=six,bg="black")
                    Snum7.config(image=seven,bg="black")
                    Snum8.config(image=eight,bg="black")
                    Snum9.config(image=nine,bg="black")
                    Snum0.config(image=zero,bg="black")
                ...
                if mainthm == 'Light':
                    ...
                    nine = ImageTk.PhotoImage(file=f"{maindir}/Dnine.png")
                    zero = ImageTk.PhotoImage(file=f"{maindir}/Dzero.png")
                    Snum1.config(image=one,bg="white")
                    Snum2.config(image=two,bg="white")
                    Snum3.config(image=three,bg="white")
                    Snum4.config(image=four,bg="white")
                    Snum5.config(image=five,bg="white")
                    Snum6.config(image=six,bg="white")
                    Snum7.config(image=seven,bg="white")
                    Snum8.config(image=eight,bg="white")
                    Snum9.config(image=nine,bg="white")
                    Snum0.config(image=zero,bg="white")
        if mainui == 'default':
            if mainthm == 'Dark':
                Snum1.config(bg="black")
                Snum2.config(bg="black")
                Snum3.config(bg="black")
                Snum4.config(bg="black")
                Snum5.config(bg="black")
                Snum6.config(bg="black")
                Snum7.config(bg="black")
                Snum8.config(bg="black")
                Snum9.config(bg="black")
                Snum0.config(bg="black")
                SCE.config(bg="#fc8700")
                SAC.config(bg="#fc8700")
                SC.config(bg="#fc8700")
            if mainthm == 'Light':
                Snum1.config(bg="white",fg="black")
                Snum2.config(bg="white",fg="black")
                Snum3.config(bg="white",fg="black")
                Snum4.config(bg="white",fg="black")
                Snum5.config(bg="white",fg="black")
                Snum6.config(bg="white",fg="black")
                Snum7.config(bg="white",fg="black")
                Snum8.config(bg="white",fg="black")
                Snum9.config(bg="white",fg="black")
                Snum0.config(bg="white",fg="black") 
                SAC.config(bg="#ff0000")
                SCE.config(bg="#ff0000")
                SC.config(bg="#ff0000") 
    except:
        pass
        ...
        if maindir:
            if mainui == 'custom':
                if mainthm == 'Dark':
                    ...
                    nine = ImageTk.PhotoImage(file=f"{maindir}/nine.png")
                    zero = ImageTk.PhotoImage(file=f"{maindir}/zero.png")
                    Wnum1.config(image=one,bg="black")
                    Wnum2.config(image=two,bg="black")
                    Wnum3.config(image=three,bg="black")
                    Wnum4.config(image=four,bg="black")
                    Wnum5.config(image=five,bg="black")
                    Wnum6.config(image=six,bg="black")
                    Wnum7.config(image=seven,bg="black")
                    Wnum8.config(image=eight,bg="black")
                    Wnum9.config(image=nine,bg="black")
                    Wnum0.config(image=zero,bg="black")
                ...
                if mainthm == 'Light':
                    ...
                    nine = ImageTk.PhotoImage(file=f"{maindir}/Dnine.png")
                    zero = ImageTk.PhotoImage(file=f"{maindir}/Dzero.png")
                    Wnum1.config(image=one,bg="white")
                    Wnum2.config(image=two,bg="white")
                    Wnum3.config(image=three,bg="white")
                    Wnum4.config(image=four,bg="white")
                    Wnum5.config(image=five,bg="white")
                    Wnum6.config(image=six,bg="white")
                    Wnum7.config(image=seven,bg="white")
                    Wnum8.config(image=eight,bg="white")
                    Wnum9.config(image=nine,bg="white")
                    Wnum0.config(image=zero,bg="white")
        if mainui == 'default':
            if mainthm == 'Dark':
                Wnum1.config(bg="black")
                Wnum2.config(bg="black")
                Wnum3.config(bg="black")
                Wnum4.config(bg="black")
                Wnum5.config(bg="black")
                Wnum6.config(bg="black")
                Wnum7.config(bg="black")
                Wnum8.config(bg="black")
                Wnum9.config(bg="black")
                Wnum0.config(bg="black")
                WCE.config(bg="#fc8700")
                WAC.config(bg="#fc8700")
                WC.config(bg="#fc8700")
            if mainthm == 'Light':
                Wnum1.config(bg="white",fg="black")
                Wnum2.config(bg="white",fg="black")
                Wnum3.config(bg="white",fg="black")
                Wnum4.config(bg="white",fg="black")
                Wnum5.config(bg="white",fg="black")
                Wnum6.config(bg="white",fg="black")
                Wnum7.config(bg="white",fg="black")
                Wnum8.config(bg="white",fg="black")
                Wnum9.config(bg="white",fg="black")
                Wnum0.config(bg="white",fg="black") 
                WAC.config(bg="#ff0000")
                WCE.config(bg="#ff0000")
                WC.config(bg="#ff0000")
    except:
        pass
        ...
        if maindir:
            if mainui == 'custom':
                if mainthm == 'Dark':
                    ...
                    nine = ImageTk.PhotoImage(file=f"{maindir}/nine.png")
                    zero = ImageTk.PhotoImage(file=f"{maindir}/zero.png")
                    Tnum1.config(image=one,bg="black")
                    Tnum2.config(image=two,bg="black")
                    Tnum3.config(image=three,bg="black")
                    Tnum4.config(image=four,bg="black")
                    Tnum5.config(image=five,bg="black")
                    Tnum6.config(image=six,bg="black")
                    Tnum7.config(image=seven,bg="black")
                    Tnum8.config(image=eight,bg="black")
                    Tnum9.config(image=nine,bg="black")
                    Tnum0.config(image=zero,bg="black")
                ...
                if mainthm == 'Light':
                    ...
                    nine = ImageTk.PhotoImage(file=f"{maindir}/Dnine.png")
                    zero = ImageTk.PhotoImage(file=f"{maindir}/Dzero.png")
                    Tnum1.config(image=one,bg="white")
                    Tnum2.config(image=two,bg="white")
                    Tnum3.config(image=three,bg="white")
                    Tnum4.config(image=four,bg="white")
                    Tnum5.config(image=five,bg="white")
                    Tnum6.config(image=six,bg="white")
                    Tnum7.config(image=seven,bg="white")
                    Tnum8.config(image=eight,bg="white")
                    Tnum9.config(image=nine,bg="white")
                    Tnum0.config(image=zero,bg="white")
        if mainui == 'default':
            if mainthm == 'Dark':
                Tnum1.config(bg="black")
                Tnum2.config(bg="black")
                Tnum3.config(bg="black")
                Tnum4.config(bg="black")
                Tnum5.config(bg="black")
                Tnum6.config(bg="black")
                Tnum7.config(bg="black")
                Tnum8.config(bg="black")
                Tnum9.config(bg="black")
                Tnum0.config(bg="black")
                TAC.config(bg="#fc8700")
                TCE.config(bg="#fc8700")
                TC.config(bg="#fc8700")
            if mainthm == 'Light':
                Tnum1.config(bg="white",fg="black")
                Tnum2.config(bg="white",fg="black")
                Tnum3.config(bg="white",fg="black")
                Tnum4.config(bg="white",fg="black")
                Tnum5.config(bg="white",fg="black")
                Tnum6.config(bg="white",fg="black")
                Tnum7.config(bg="white",fg="black")
                Tnum8.config(bg="white",fg="black")
                Tnum9.config(bg="white",fg="black")
                Tnum0.config(bg="white",fg="black") 
                TCE.config(bg="#ff0000")
                TAC.config(bg="#ff0000")
                TC.config(bg="#ff0000") 
    except:
        pass
else:
